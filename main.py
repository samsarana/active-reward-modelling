...
def do_random_experiment(env, args, writer1, writer2):
    for i_train_round in range(args.n_rounds):
        ...
        env.reset()
        for step in trange(args.n_agent_steps, desc='Taking random actions for {} steps'.format(args.n_agent_steps), dynamic_ncols=True, file=sys.stdout, miniters=args.n_agent_steps):
...
def do_pretraining(env, q_net, reward_model, prefs_buffer, args, obs_shape, act_shape, writer1, writer2):
    ...
    state = env.reset()
    for _ in trange(n_initial_steps, desc='Stage 0.1: Collecting rollouts from untrained policy, {} agent steps'.format(n_initial_steps), dynamic_ncols=True, file=sys.stdout, miniters=n_initial_steps):
    ...
    for epoch in trange(args.n_epochs_pretrain_rm, desc='Stage 0.3: Intialise and pretrain reward model for {} batches on those preferences'.format(args.n_epochs_pretrain_rm), dynamic_ncols=True, file=sys.stdout, miniters=args.n_epochs_pretrain_rm):
...
def do_training(env, q_net, q_target, reward_model, prefs_buffer, args, obs_shape, act_shape, writer1, writer2):
    ...
    for i_train_round in range(args.n_rounds):
        ...
        reward_model.train() # dropout on
        for epoch in trange(args.n_epochs_train_rm, desc='Stage 1.3: Train reward model for {} batches on those preferences'.format(args.n_epochs_train_rm), dynamic_ncols=True, file=sys.stdout, miniters=args.n_epochs_train_rm):
