...
def parse_arguments():
    ...
    parser.add_argument('--batch_size_acq', type=int, nargs='+', default=-1, help='In acquiring `n_labels_per_round`, what batch size are these acquired in? Reward model is trained after every acquisition batch. If -1 (default), `batch_size_acq` == `n_labels_per_round`, as in Christiano/Ibarz')
